...
import { AlertTriangle, CheckCircle, XCircle, Loader, Search, Shield, Download, Clock, Calendar, Globe, ArrowLeft } from 'lucide-react';
...
export default function DomainSecurityChecker() {
  ...
  const [domainResults, setDomainResults] = useState<DomainResults | null>(null);
  ...
  const checkHTTPS = async (url: string) => {
    try {
  ...
  const checkHSTS = async (domain: string) => {
  ...
  const checkSecurityHeaders = async (url: string) => {
    try {
      ...
      return {
        ...
        note: 'ℹ️ Security headers cannot be checked due to browser CORS restrictions (this is normal). The domain may still have proper security headers configured.'
      };
    }
  };

  const calculateDomainScore = (https: any, hsts: any, headers: any) => {
    let score = 0;
  ...
  const getMockWhoisData = (_domain: string) => {
